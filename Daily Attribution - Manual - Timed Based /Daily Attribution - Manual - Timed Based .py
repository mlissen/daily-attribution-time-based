...
while current_date <= end_date:
    ...
    try:
        enriched_coupon_attributed_data = ad_groups_join.join(
            ads_data_df,
    ...
    # Log final data rows
    sample_rows_final = enriched_coupon_attributed_data.limit(2).collect()
    for row in sample_rows_final:
        logger.info(f"LISS Final Sample row data after ads join: {row.asDict()}")
    ...
    log_sample_data(final_data, "Enriched data:")
    ...
    time_based_conversions = time_based_conversions.withColumn(
        "sales_date",
        lit(current_date)  # Use the current_date from your loop
    ).withColumn("attribution_method", lit("time_based_incremental"))
